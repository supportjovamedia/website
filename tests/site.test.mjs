...
if (!["localhost", "127.0.0.1"].includes(new URL(base).hostname))
  throw new Error("Tests must target a local preview.");
test("every sitemap page responds with its canonical URL and one main heading", async () => {
  ...
  for (const route of routes) {
    ...
    assert.ok(canonical, route + " canonical exists");
    assert.equal(
...
test("unknown pages and service slugs return real 404 responses", async () => {
  for (const path of ["/does-not-exist", "/services/does-not-exist"])
    assert.equal((await fetch(base + path)).status, 404, path);
});
test("terms alias remains available with its primary canonical URL", async () => {
  const response = await fetch(base + "/terms-of-service");
...
test("contact endpoint truthfully reports unconfigured delivery", async () => {
  const response = await fetch(base + "/api/contact", { method: "POST" });
