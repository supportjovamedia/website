import { startNewsletterAutomation } from './newsletter-automation.mjs';
...
export function createGrowthHandler({env=process.env,send=fetch,now=Date.now}={}){
 ...
 if(existing.ok){contactId=existing.data.id;if(existing.data.unsubscribed)return reply(409,{error:"This address previously unsubscribed. Please contact us if you want to rejoin; this link will not override your preference."});
 ...
 }else if(existing.status===404){const created=await api("/contacts","POST",{email:contact.email,unsubscribed:false,segments:[{id:env.RESEND_NEWSLETTER_SEGMENT_ID}],properties:{newsletter_consent_at:new Date(time).toISOString(),newsletter_consent_version:"footer-v1"}});if(!created.ok)throw Error();contactId=created.data.id;}else throw Error();
 if(!contactId)throw Error();
 try{await startNewsletterAutomation({env,send,email:contact.email,contactId});}catch{return reply(502,{error:"Your subscription is confirmed, but we could not finish setting up your welcome emails. Please contact us if they do not arrive."});}
 ...
 const existing=await api("/contacts/"+encodeURIComponent(email),"GET");
 if(existing.ok){
 if(existing.data.unsubscribed)return reply(409,{error:"You previously unsubscribed. Please contact us if you would like to rejoin."});
 const consent=existing.data.properties;
 if(consent?.newsletter_consent_at?.value&&consent?.newsletter_consent_version?.value==="footer-v1")return reply(200,{ok:true,status:"already_subscribed"});
 }else if(existing.status!==404)throw Error();
 }
 ...
 if(!result.ok||!result.data.id)throw Error();
 return reply(200,{ok:true});
