import { validateEnquiry, countryOptions } from "./enquiry-fields.mjs";
...
export function createContactHandler({ env = process.env, send = fetch, now = Date.now } = {}) {
  ...
  return async function POST(request) {
    ...
    const {values, errors} = validateEnquiry(data);
    if (Object.keys(errors).length || data.website)
      return reply(400,{ok:false,error:"Please check the highlighted form fields.",fields:errors});
    ...
    const payload={from:env.CONTACT_FROM_EMAIL,to:[env.CONTACT_TO_EMAIL],reply_to:values.email,subject:"New JovaMedia website enquiry",text:
      `Name: ${values.name}\nEmail: ${values.email}\nCompany: ${values.company||"Not provided"}\nCountry: ${countryOptions.find(country => country.code === values.country)?.name || "Not provided"}\nMonthly budget (GBP): ${values.budget || "Not applicable"}\nInterested in: ${values.service}${values.other ? ` — ${values.other}` : ""}\nTiming: ${values.timing || "Not specified"}\n\n${values.message}`};
