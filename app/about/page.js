  ...
  return (
    <>
      <section className="relative overflow-hidden">
        <div
          aria-hidden="true"
          className="pointer-events-none absolute -right-20 -top-20 h-80 w-80 animate-float-slow rounded-full bg-gold/10 blur-3xl"
        />
        <div className="relative mx-auto max-w-6xl px-6 pb-16 pt-16 lg:px-8 lg:pt-24">
          <Reveal>
            <span className="text-xs font-semibold uppercase tracking-[0.25em] text-gold">
              About Jova Media
            </span>
            <h1 className="mt-4 max-w-3xl font-serif-brand text-4xl leading-tight text-navy sm:text-5xl">
              We started Jova Media because too many agencies talk a good game and deliver a slow
              one.
            </h1>
            <p className="mt-6 max-w-2xl text-lg leading-relaxed text-navy/70">
              Founded in London, Jova Media brings strategy, content and paid media together under
              one roof so nothing gets lost in translation between departments, freelancers or
              vendors. We work as an extension of our clients&rsquo; teams &mdash; close enough to
              move fast, senior enough to be trusted with the decisions that matter.
            </p>
          </Reveal>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-6xl px-6 py-20 lg:px-8 lg:py-28">
        <div className="grid grid-cols-1 gap-12 lg:grid-cols-2">
          <Reveal>
            <SectionHeading
              eyebrow="Our story"
              title="From a two-person team to a full-service partner."
            />
            <p className="mt-6 text-base leading-relaxed text-navy/70">
              ...
              content, SEO, brand strategy and web development as clients asked us to take on more
              of their growth.
            </p>
          ...
          <div className="mt-14 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {teamValues.map((value, i) => (
              <Reveal key={value.title} delay={i * 100}>
                <div className="h-full rounded-2xl border border-navy/10 bg-ivory p-6 text-center transition-all duration-300 hover:-translate-y-1 hover:border-gold/40 hover:shadow-lg hover:shadow-navy/5">
        ...
        <div className="mt-12 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {teamMembers.map((member, i) => (
            <Reveal key={member.name} delay={(i % 3) * 100}>
              <div className="flex h-full items-center gap-4 rounded-2xl border border-navy/10 p-5 transition-all duration-300 hover:-translate-y-1 hover:border-gold/40 hover:shadow-lg hover:shadow-navy/5">
